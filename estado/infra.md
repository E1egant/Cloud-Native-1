...
## Falta
**Reescribir `docker-compose.apps.yml`, `Dockerfile.service`, `deploy.sh` y CI para repos separados** (hoy usan `context: ..` del monorepo); clúster RabbitMQ 2 nodos y Kafka 3 brokers + ZK del caso; App Registration con el set de roles acordado (T1).
